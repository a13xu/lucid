...
Write-Host "✅ MCP server built  ->  build/index.js"
Write-Host ""
...
if ($claudeCmd) {
  if (Prompt-YN "Register Lucid as Claude Code MCP server?" "Y") {
    ...
    Write-Host "✅ Registered: lucid -> node $nodeArg"
  } else {
    Write-Host "⏭️  Skipped MCP registration (run manually later)"
    Write-Host "   claude mcp add --transport stdio lucid -- node `"$ScriptDir\build\index.js`""
  }
} else {
  Write-Host "⚠️  claude CLI not found — register manually after install:"
  Write-Host "   claude mcp add --transport stdio lucid -- node `"$ScriptDir\build\index.js`""
}
Write-Host ""
...
if (Prompt-YN "Install Web Dev Cycle Manager?" "N") {
  ...
  if ($built) {
    ...
    Write-Host "✅ Web built  ->  dist/client/  +  dist/server/"
  }

  Write-Host ""
  Write-Host "✅ Web Dev Cycle Manager gata!"
  Write-Host ""
  ...
  Write-Host "⏭️  Web UI skipped"
  Write-Host "   Pentru a instala mai tarziu: powershell -ExecutionPolicy Bypass -File `"$ScriptDir\install.ps1`""
}
